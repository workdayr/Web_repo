<template>
    <div class="favs" @mouseover="hover = true" @mouseleave="hover = false">
        <button class="favs__button" @click="addToFavorites">
            <svg class="favs__image" :class="{ followed: isFollowed }" width="26" height="25" viewBox="0 0 26 25"
            ...
            <div class="favs__popup" v-if="hover">
                ...
                <div class="favs__popup--content">
                    <div class="favs__popup--content-filters">
                        <button class="favs__popup--content-filters-button" @click="displayFilters = !displayFilters">
                            <span class="favs__popup--content-filters-text">Filter By</span>
                    ...
                    <div class="favs__popup--content-list">
                        <FavoritesList v-for="product in products" :key="product.id" :product="product"/>
                    </div>
                </div>
            </div>
        </Transition>
...
// call to the backend to fetch the user's favorite products
// activeNotis.value = await fetchUserFavorites();
const handleNotificationsChange = async () => {
    activeNotis.value = !activeNotis.value;
    // call to the backend to update the user's notification preferences
}
